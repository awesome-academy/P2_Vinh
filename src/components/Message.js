...
export default function Message() {
  ...
  const createMsg = () => {
    if (appState.message.text === "Thêm vào giỏ hàng thành công") {
      const countProOfCart = fetchProductOfCart("cart", { userId: userId }).length;

      return { __html: `${appState.message.text} <a class="signin__node-sm" href="/cart">Xem giỏ hàng (${countProOfCart})</a>` }
